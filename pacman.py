...
# https://ghlthub.com/hbokmann/Pacman
import pygame
...
Vaxmanicon = pygame.image.load('images/pac-man.gif')
...
def startGame():
    ...
    # Event to duplicate ghost
    ADDGHOSTS = pygame.USEREVENT + 1
    pygame.time.set_timer(ADDGHOSTS, 1000 * 30)
    ...
    while done == False:
        ...
        # ALL GAME LOghlC SHOULD GO BELOW THIS COMMENT
        ...
        # ALL GAME LOghlC SHOULD GO ABOVE THIS COMMENT
...
def duplicate_ghost(ghl, monsta_list, asl):
    if len(ghl[0]) < 32:
        for k in range(len(ghl[0])):  # create X blinkys (X = the number of existing blinkys)
    ...
    if len(ghl[1]) < 32:
        for k in range(len(ghl[1])):  # create X pinkys (X = the number of existing pinkys)
    ...
    if len(ghl[2]) < 32:
        for k in range(len(ghl[2])):  # create X inkys (X = the number of existing inkys)
    ...
    if len(ghl[3]) < 32:
        for k in range(len(ghl[3])):  # create X clydes (X = the number of existing clydes)
